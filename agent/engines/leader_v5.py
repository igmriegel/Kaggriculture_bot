...
_V5_SEED_COST = {"WHEAT": 10, "CARROT": 20, "TOMATO": 50, "STRAWBERRY": 100, "MELON": 80}
...
class LeaderV5Config(LeaderV2Config):
...
class LeaderV5Engine(LeaderV2Engine):
    ...
    def __init__(self, config: LeaderV5Config | None = None) -> None:
        super().__init__(config or LeaderV5Config())
        self.v5_config = config or LeaderV5Config()
    ...
    def _goals(self, state: NormalizedState) -> tuple[ProductionGoal, ...]:
        ...
        if horizon < 8 or (cow_ev <= 0 and sheep_ev <= 0):
            ...
            target_animals = min(max_pastures, self.v5_config.max_animals)
    ...
    def _tasks(self, state: NormalizedState, goals: tuple[ProductionGoal, ...]) -> list[Task]:
        ...
        planned_animals = (
            self.v5_config.opening_animals
    ...
    def _build_market_orders(
        ...
        if state.day == 0 and state.hour == 1:
            ...
            if seed_budget >= 10 * _V5_SEED_COST["MELON"] + 6 * _V5_SEED_COST["WHEAT"]:
                ...
                for goal in goals:
                    if goal.name.startswith("plant_"):
                        ...
                        cost_per_seed = _V5_SEED_COST.get(crop_name, 20)
        ...
        if self._closing(state):
            return orders[: self.v5_config.max_orders]
        ...
        if feed_needed > 0 and (state.hour in {0, 1} or state.shed.get("WHEAT", 0) == 0):
            if spending >= wheat_price and len(orders) < self.v5_config.max_orders:
                qty = min(
        ...
        if state.hour in {0, 1} and len(orders) < self.v5_config.max_orders:
            ...
            if state.money > 10_000:
                target_workers = self.v5_config.target_hands_midgame
            ...
            while (
                ...
                and len(orders) < self.v5_config.max_orders
        ...
        if current_animals < animal_goal and self._animal_chain_ready(state):
            ...
            if spending >= (cost + feed_buffer_cost) and len(orders) < self.v5_config.max_orders:
                orders.append(["BUY_ANIMAL", next_animal, 1])
                spending -= cost

        # 5. Seed purchases based on portfolio goals
        for goal in goals:
            if not goal.name.startswith("plant_") or len(orders) >= self.v5_config.max_orders:
                continue
            crop = goal.name.removeprefix("plant_").upper()
            shortfall = max(0, goal.quantity - state.seeds.get(crop, 0))
            if shortfall <= 0:
                continue
            seed_cost = _V5_SEED_COST.get(crop, 20)
    ...
    def _sales(
        ...
        capacity_pressure = sum(state.shed.values()) >= (
            state.shed_capacity - self.v5_config.shed_safety_buffer
        )
        ...
        for item, amount in sorted(state.shed.items()):
            ...
            values = marginal_sale_values(
                ...
                sellable,
                opponent_buffer=self.v5_config.opponent_market_buffer,
            )
...
__all__ = ["LeaderV5Config", "LeaderV5Engine"]
