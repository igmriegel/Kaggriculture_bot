...
def main():
    ...
    v7_report = run_evaluation(
        "leader-v9-2", "leader-v7", LeaderV92Engine, LeaderV7Engine, num_matches=args.num_matches
    )
    v8_report = run_evaluation(
        "leader-v9-2", "leader-v8", LeaderV92Engine, LeaderV8Engine, num_matches=args.num_matches
    )
    v9_report = run_evaluation(
        "leader-v9-2", "leader-v9", LeaderV92Engine, LeaderV9Engine, num_matches=args.num_matches
    )
    v9_1_report = run_evaluation(
        "leader-v9-2", "leader-v9-1", LeaderV92Engine, LeaderV91Engine, num_matches=args.num_matches
    )
    ...
    with open("reports/benchmarks/latest.md", "w") as f:
        f.write("# Consolidated Benchmarks Report (V9.2)\n\n")

        f.write("## Summary\n\n")
        f.write(
            "| Matchup | Win Rate | Avg V9.2 | Avg Opp | Min V9.2 | Max V9.2 | "
            "Min Opp | Max Opp | Margin |\n"
        )
        f.write("|:---|:---:|---:|---:|---:|---:|---:|---:|---:|\n")
        for key in ["v7", "v8", "v9", "v9_1"]:
            r = all_reports[key]
            f.write(
                f"| V9.2 vs {r['opponent'].upper()} | {r['win_rate']}% "
        ...
        for key in ["v7", "v8", "v9", "v9_1"]:
            ...
            for m in rep["matches"]:
                badge = (
                    f"**{m['result']}**"
                    if "WIN" in m["result"] and "V9-2" in m["result"]
                    else m["result"]
                )
                f.write(
    ...
    print("\n=== SUMMARY TABLE ===")
    header = (
        f"{'Matchup':<18} {'Win Rate':>8} {'Avg V9.2':>10} {'Avg Opp':>10} "
        f"{'Min V9.2':>10} {'Max V9.2':>10} {'Min Opp':>10} {'Max Opp':>10} {'Margin':>10}"
    )
    print(header)
    print("-" * len(header))
    for key in ["v7", "v8", "v9", "v9_1"]:
        r = all_reports[key]
        print(
            f"V9.2 vs {r['opponent'].upper():<12} {r['win_rate']:>7.1f}% "
