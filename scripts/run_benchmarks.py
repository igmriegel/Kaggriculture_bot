...
def main():
    ...
    v6_report = run_evaluation(
        "leader-v9-1", "leader-v6", LeaderV91Engine, LeaderV6Engine, num_matches=args.num_matches
    )
    v7_report = run_evaluation(
        "leader-v9-1", "leader-v7", LeaderV91Engine, LeaderV7Engine, num_matches=args.num_matches
    )
    v8_report = run_evaluation(
        "leader-v9-1", "leader-v8", LeaderV91Engine, LeaderV8Engine, num_matches=args.num_matches
    )
    v9_report = run_evaluation(
        "leader-v9-1", "leader-v9", LeaderV91Engine, LeaderV9Engine, num_matches=args.num_matches
    )
    v10_report = run_evaluation(
        "leader-v9-1", "leader-v10", LeaderV91Engine, LeaderV10Engine, num_matches=args.num_matches
    )
    ...
    with open("reports/benchmarks/latest.md", "w") as f:
        f.write("# Consolidated Benchmarks Report (V9.1)\n\n")

        f.write("## Summary\n\n")
        f.write(
            "| Matchup | Win Rate | Avg V9.1 | Avg Opp | Min V9.1 | Max V9.1 | "
    ...
    header = (
        ...
        f"{'Min V9.1':>10} {'Max V9.1':>10} {'Min Opp':>10} {'Max Opp':>10} {'Margin':>10}"
    )
    print(header)
    print("-" * len(header))
    for key in ["v6", "v7", "v8", "v9", "v10"]:
