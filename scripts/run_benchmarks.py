...
def main():
    ...
    v10_report = run_evaluation(
        "leader-v11", "leader-v10", LeaderV11Engine, LeaderV10Engine, num_matches=args.num_matches
    )
    ...
    with open("reports/benchmarks/latest.md", "w") as f:
        ...
        f.write("## Summary\n\n")
        f.write(
            "| Matchup | Win Rate | Avg Agent | Avg Opp | Min Agent | Max Agent | "
            "Min Opp | Max Opp | Margin | Set Time |\n"
        )
        f.write("|:---|:---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
        for key in ["v7", "v8", "v9", "v9_1", "v10", "mcts_v10"]:
            r = all_reports[key]
            f.write(
    ...
    print("\n=== SUMMARY TABLE ===")
    header = (
        f"{'Matchup':<22} {'Win Rate':>8} {'Avg Agent':>10} {'Avg Opp':>10} "
        f"{'Min Agent':>10} {'Max Agent':>10} {'Min Opp':>10} {'Max Opp':>10} "
    ...
    for key in ["v7", "v8", "v9", "v9_1", "v10", "mcts_v10"]:
        r = all_reports[key]
        print(
            f"{r['agent'].upper()} vs {r['opponent'].upper():<13} {r['win_rate']:>7.1f}% "
